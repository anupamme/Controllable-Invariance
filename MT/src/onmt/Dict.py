...
class Dict(object):
    ...
    def add_dict(self, d, times=None): # times = # token in self / # token in d
        if times is not None:
            tot_num_d = sum(d.frequencies.values())
            tot_num = sum(self.frequencies.values())
            ratio = tot_num * 1.0 / (tot_num_d * times)
            print "ratio", ratio
        else:
            ratio = 1.
        for label in d.labelToIdx:
            self.add(label, None, ratio * d.frequencies[d.labelToIdx[label]])
