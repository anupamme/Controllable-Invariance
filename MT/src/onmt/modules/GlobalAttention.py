...
class MLPAttention(nn.Module):
    def __init__(self, dim):
        ...
        self.query_in = nn.Linear(dim, dim)
        self.linear_attn = nn.Linear(dim, 1)
        self.sm = nn.Softmax()
        self.linear_out = nn.Linear(dim*2, dim, bias=False)
        self.tanh = nn.Tanh()
    ...
    def forward(self, input, context):
        ...
        context: batch x sourceL x dim
        """
        projInput = self.query_in(input).unsqueeze(1).expand_as(context)  # batch x sourceL x dim
        hidden = self.tanh(context + projInput)                           # batch x sourceL x dim
        hidden2 = hidden.view(-1, context.size(2))                        # batch * sourceL x dim

        # Get attention
        attn = self.linear_attn(hidden2).view(context.size(0), context.size(1))  # batch x sourceL
        if self.mask is not None:
            attn.data.masked_fill_(self.mask, -_INF)
        attn = self.sm(attn)
        attn3 = attn.view(attn.size(0), 1, attn.size(1))  # batch x 1 x sourceL
